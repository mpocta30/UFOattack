...
wallObj.prototype.draw = function() {
    ...
    for(var i = 0; i < 5; i++) {
        for(var j = 0; j < 10; j++) {
            fill(bricks[counter].r, bricks[counter].g, bricks[counter].b);
            rect(this.x+i*floor(80/10), this.y+j*floor(40/10), floor(80/10), floor(40/10));
            counter += 1;
        }
    }
    fill(255, 255, 255);
...
playerObj.prototype.draw = function() {
    if (keyArray[RIGHT] === 1) {
        ...
        if ((this.x > ((400 - scrollval.x) + (0 - scrollval.x))/2) && (scrollval.x > -600)) {
...
var keyPressed = function() {
    keyArray[keyCode] = 1;
    if(keyCode === RIGHT && gamestate === 1) {
        scrollval.x -= 5;   
    }
    if(keyCode === LEFT  && gamestate === 1) {
        scrollval.x += 5;   
    }
};
